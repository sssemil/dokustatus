...
import { AlertTriangle, ArrowUp, ArrowDown, Loader2, CreditCard, ExternalLink } from 'lucide-react';
...
type ModalState = 'loading' | 'preview' | 'processing' | 'requires_action' | 'success' | 'error';
...
export function PlanChangeModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/public/domain/${apiDomain}/billing/plan-change`, {
        method: 'POST',
        headers: {
      ...
      if (data.change_type === 'downgrade') {
        ...
        setState('success');
      } else if (data.payment_intent_status === 'requires_action' && data.client_secret) {
        ...
        setError('Your payment method was declined. Please update your payment method and try again.');
        setState('error');
      } else {
            ...
            <div className="p-4 bg-zinc-800/50 rounded-lg border border-zinc-700">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-zinc-400">
                  <span>From</span>
                  <span>{currentPlan.name} ({formatPrice(currentPlan.price_cents, currentPlan.currency)})</span>
                </div>
                <div className="flex justify-between text-zinc-400">
                  <span>To</span>
                  <span>{newPlan.name} ({formatPrice(newPlan.price_cents, newPlan.currency)})</span>
                </div>
              </div>
            </div>
            ...
            {isUpgrade && preview && (
              <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-blue-300">Amount due today</span>
                </div>
                ...
                {preview.period_end > 0 && (
                  <p className="text-sm text-zinc-400 mt-2">
                    Next full payment on {formatEffectiveDate(preview.period_end)}
                  </p>
                )}
              </div>
            )}
            ...
            {!isUpgrade && preview && (
              <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-yellow-300">Scheduled change</span>
                </div>
                <ul className="text-sm text-zinc-300 space-y-1 ml-6 list-disc">
                  <li>Your {currentPlan.name} access continues until {formatEffectiveDate(preview.effective_at)}</li>
                  <li>On {formatEffectiveDate(preview.effective_at)}, you'll be charged {formatPrice(newPlan.price_cents, newPlan.currency)} for {newPlan.name}</li>
        ...
        return (
          <div className="flex flex-col items-center py-8">
            ...
            <p className="mt-4 text-zinc-400">
              {state === 'requires_action' ? 'Completing payment...' : 'Processing...'}
            </p>
          </div>
        );
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg text-center">
              ...
              <p className="font-medium text-green-300">
                {result?.change_type === 'upgrade' ? 'Plan upgraded successfully!' : 'Plan change scheduled!'}
              </p>
              {result?.change_type === 'downgrade' && result.effective_at && (
                <p className="text-sm text-zinc-400 mt-2">
                  Your plan will change on {formatEffectiveDate(result.effective_at)}
                </p>
              )}
              {result?.amount_charged_cents && result.currency && (
                <p className="text-sm text-zinc-400 mt-2">
                  Charged: {formatPrice(result.amount_charged_cents, result.currency)}
                </p>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-red-300">Failed to change plan</span>
              </div>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="primary" onClick={() => { setState('loading'); }} className="flex-1">
                Try Again
              </Button>
            </div>
          </div>
  ...
  const title = isUpgrade ? `Upgrade to ${newPlan.name}` : `Downgrade to ${newPlan.name}`;

  return (
    <Modal
      open={open}
      onClose={state === 'processing' || state === 'requires_action' ? () => {} : handleClose}
      title={title}
      size="md"
    >
      {renderContent()}
