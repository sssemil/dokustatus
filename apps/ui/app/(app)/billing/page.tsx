...
export default function BillingPage() {
  ...
  const [selectedPlanForDummy, setSelectedPlanForDummy] =
    useState<SubscriptionPlan | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [subRes, plansRes, providersRes] = await Promise.all([
        fetch(`/api/public/domain/${apiDomain}/billing/subscription`, {
          credentials: "include",
        }),
        fetch(`/api/public/domain/${apiDomain}/billing/plans`, {
          credentials: "include",
        }),
        fetch(`/api/public/domain/${apiDomain}/billing/providers`, {
          credentials: "include",
        }),
      ]);
      ...
      if (providersRes.ok) {
        ...
        setEnabledProviders(
          providersData.filter((p: EnabledPaymentProvider) => p.is_active),
        );
      }
  ...
  const currentPlan =
    hasActiveSubscription && subscription?.plan_code
      ? plans.find((p) => p.code === subscription.plan_code)
      : null;

  // Provider helpers
  const hasStripeProvider = enabledProviders.some(
    (p) => p.provider === "stripe",
  );
  ...
  const handleSubscribe = async (planCode: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/public/domain/${apiDomain}/billing/checkout`,
        {
          method: "POST",
  ...
  const handleManageSubscription = async () => {
    try {
      const res = await fetch(
        `/api/public/domain/${apiDomain}/billing/portal`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      const res = await fetch(
        `/api/public/domain/${apiDomain}/billing/cancel`,
        {
          method: "POST",
          credentials: "include",
        },
          ...
          <div className="flex items-center justify-between p-4 bg-zinc-800/50 rounded-lg border border-zinc-700">
                ...
                {subscription.trial_end &&
                  subscription.status === "trialing" && (
                    <span className="ml-2 text-blue-400">
                      Trial ends {formatPeriodEnd(subscription.trial_end)}
                    </span>
                  )}
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              {!subscription.cancel_at_period_end && (
                <HoldButton
                  onComplete={handleCancelSubscription}
                  variant="danger"
                  duration={2000}
                  disabled={canceling}
                >
          ...
          {subscription.cancel_at_period_end && (
            <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              <p className="text-sm text-yellow-200">
                ...
                billing period. You will retain access until{" "}
                {formatPeriodEnd(subscription.current_period_end)}.
              </p>
            </div>
          )}
                ...
                {payments.map((payment) => (
                  ...
                  >
                        ...
                        <Badge
                          variant={
                            payment.payment_provider === "stripe"
                              ? "default"
                              : payment.payment_provider === "dummy"
                                ? "warning"
                                : "info"
                          }
                        >
                    ...
                    <td className="py-3 px-2">
                      <Badge
                        variant={
                          getPaymentStatusBadgeColor(payment.status) === "green"
                            ? "success"
                            : getPaymentStatusBadgeColor(payment.status) ===
                                "red"
                              ? "error"
                              : getPaymentStatusBadgeColor(payment.status) ===
                                  "yellow"
          ...
          {paymentsPagination.total_pages > 1 && (
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-zinc-800">
              <span className="text-sm text-zinc-400">
                Page {paymentsPagination.page} of{" "}
        ...
        <Card className="p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {plans.map((plan) => {
              ...
              const isCurrentPlan =
                hasActiveSubscription && subscription?.plan_code === plan.code;
              return (
                <div
                  key={plan.id}
                  className={`
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {isCurrentPlan && <Badge variant="info">Current</Badge>}
                  </div>
