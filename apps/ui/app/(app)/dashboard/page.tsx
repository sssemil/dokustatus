...
import { AlertTriangle, Globe, Users, ChevronRight, Plus, Activity } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [domainsNeedingAttention, setDomainsNeedingAttention] = useState<Domain[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (domainsRes.ok) {
          ...
          const needAttention = domains.filter(d =>
            d.status !== 'verified' || (d.status === 'verified' && !d.has_auth_methods)
          );
        ...
        <Card className="p-4 sm:p-5 border-amber-600/30 bg-amber-900/10">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-amber-400 mb-2">Domains need attention</h3>
                ...
                {domainsNeedingAttention.map(d => (
                  ...
                  >
                    ...
                    <span className="text-xs text-amber-400 flex-shrink-0 ml-2">
                      {d.status !== 'verified' ? 'DNS not verified' : 'No auth methods'}
                    </span>
                  </Link>
                ))}
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card className="p-4 sm:p-5">
          ...
          {loading ? (
            ...
            <div className="text-2xl sm:text-3xl font-bold mb-2">{planLimits.authentications.used}</div>
          )}
          <ProgressBar value={planLimits.authentications.used} max={planLimits.authentications.limit} className="mb-2" />
          <div className="text-xs text-zinc-500">
            {planLimits.authentications.limit - planLimits.authentications.used} remaining this month
          </div>
        </Card>
        ...
        <Card className="p-4 sm:p-5">
          ...
          {loading ? (
            ...
            <div className="text-2xl sm:text-3xl font-bold mb-2">{stats?.domains_count ?? 0}</div>
          )}
          <ProgressBar value={stats?.domains_count ?? 0} max={domainsLimit} className="mb-2" />
          <div className="text-xs text-zinc-500">
        ...
        <Card className="p-4 sm:p-5">
          ...
          {loading ? (
            ...
            <div className="text-2xl sm:text-3xl font-bold mb-2">{stats?.total_users ?? 0}</div>
          )}
          <ProgressBar value={stats?.total_users ?? 0} max={usersLimit} className="mb-2" />
          <div className="text-xs text-zinc-500">
