...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const fetchUser = async () => {
    ...
    try {
      let res = await fetch(`/api/public/domain/${apiDomain}/auth/session`, {
        credentials: "include",
      });
      ...
      if (res.status === 401) {
        const refreshRes = await fetch(
          `/api/public/domain/${apiDomain}/auth/refresh`,
          {
            method: "POST",
            credentials: "include",
          },
        ...
        if (refreshRes.ok) {
          ...
          res = await fetch(`/api/public/domain/${apiDomain}/auth/session`, {
            credentials: "include",
          });
        }
      }
  ...
  if (isIngress) {
    ...
    return (
      <AppContext.Provider
        value={{ user, refetchUser: fetchUser, isIngress, displayDomain }}
      >
        <ToastProvider>
          <main className="flex items-center justify-center p-8">
            <div
              className={
                pathname === "/billing" ? "max-w-2xl w-full" : "max-w-md w-full"
              }
            >
              {children}
            </div>
