    ...
    const consumeToken = async () => {
      ...
      try {
        const res = await fetch(
          `/api/public/domain/${apiDomain}/auth/verify-magic-link`,
          {
            method: "POST",
        ...
        if (res.ok) {
          ...
          try {
            ...
            if (errorData.code === "SESSION_MISMATCH") {
              ...
              setErrorMessage(
                "Please use the same browser or device where you requested the login link.",
              );
            } else {
          ...
          try {
            const errorData: { error_code?: string; message?: string } =
              await res.json();
            setStatus("error");
