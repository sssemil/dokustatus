...
function GoogleCallbackHandler() {
    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/public/domain/${apiDomain}/auth/google/exchange`,
          {
            method: "POST",
        ...
        if (res.ok) {
          ...
          if (res.status === 410 || errData.code === "OAUTH_RETRY_EXPIRED") {
            ...
            setErrorMessage(
              "Your sign-in session expired. Please restart the login process.",
            );
            ...
            setErrorMessage(
              errData.message || "Failed to complete Google sign-in.",
            );
          }
        }
      } catch {
  ...
  const handleConfirmLink = async () => {
    ...
    try {
      const res = await fetch(
        `/api/public/domain/${apiDomain}/auth/google/confirm-link`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleCancelLink = () => {
    setStatus("error");
    setErrorMessage(
      "Google account linking was cancelled. You can try a different sign-in method.",
    );
  };
        ...
        {status === "needs_link" && linkData && (
          <>
            ...
            <p className="text-zinc-400 mt-2 mb-6">
              An account with the email <strong>{linkData.email}</strong>{" "}
              already exists. Would you like to link your Google account to it?
            </p>
